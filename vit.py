...
# TT_nonlocal101_k400_True  TT_slowfast101_k400_True  TT_tpn101_k400_True   
# Meta_nonlocal101_k400_true_10_20_4_4  Meta_slowfast101_k400_true_10_20_4_4   Meta_tpn101_k400_true_10_20_4_4


def arg_parse():
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--adv_path', type=str, default='/data16t/tangchen/pangbo/video_data/ResPA_tpn_k400_True/', help='the path of adversarial examples.')
...
class SwinTransformer3D(nn.Module):
    ...
    def forward(self, x,ft=False):
        ...
        x=self.cls_head.forward(x,ft)
        return x[0]
    def head(self,x):
        x,_=self.cls_head.forward(x)
        return x
...
if __name__ == "__main__":
    ...
    model_path="/data16t/tangchen/pangbo/vit_video_pretrained/swin_small_patch244_window877_kinetics400_1k.pth"
    checkpoints=torch.load(model_path)['state_dict']
    from collections import OrderedDict
    state_dict = checkpoints['state_dict'] if 'state_dict' in checkpoints else checkpoints

    # for k in list(state_dict.keys())[:10]:
    #     print(k)
        
    # 移除 'backbone.' 前缀的新 state_dict
    ...
    torch.save({'state_dict': new_state_dict}, '/data16t/tangchen/pangbo/vit_video_pretrained/small-swin.pth')
    # 打印新的键以检查
    ...
    with open(os.path.join(args.adv_path, 'average_success_rate_videoswin.json'), 'w') as opt:
        json.dump(model_attack_rate, opt)
    
    info_df.to_csv(os.path.join(args.adv_path, 'results_all_models_prediction_videoswin.csv'), index=False)
    with open(os.path.join(args.adv_path, 'top1_acc_all_models_videoswin.json'), 'w') as opt:
        json.dump(model_val_acc, opt)
        
    consist_data = {
        'gt_label': labels,  # 第一列：真实标签
        'motionformer': preds     # 第二列：预测值
    }
